...
Arya is a multi-product on-chain platform built on Stellar. Instead of shipping a single contract with a single use case, this project combines several related products into one ecosystem:

- crowdfunding for community fundraising campaigns
- launchpad sales for token distributions
- staking for ARYA holders
- treasury and token liquidity operations
- admin tooling for managing shared platform configuration

The core idea is that these products should not behave like isolated apps. They share common platform infrastructure:

- one registry contract for canonical addresses
- one treasury destination for protocol revenue
- one staking system for reward distribution
- one frontend that makes the whole suite easier to discover and use

In practice, that means crowdfunding and launchpad do more than just collect funds. When protocol fees are taken, part of the fee goes to the treasury and part goes to staking, so activity in one part of the ecosystem can benefit ARYA stakers.

## Product Story

You can think of Arya as a small on-chain economy with clear roles:

- users browse campaigns and launchpad sales from the frontend
- creators open crowdfunding campaigns or launchpad sales
- contributors fund those campaigns or sales using one supported asset per listing
- ARYA holders stake tokens to earn protocol rewards
- the treasury manages ARYA/XLM liquidity and receives the treasury share of protocol fees
- the admin manages platform-level settings and issuer-to-treasury token funding

This creates a full loop:

1. ARYA is issued and treasury liquidity is seeded
2. users trade and hold ARYA
3. users participate in crowdfunding and launchpad activity
4. protocol fees are split between treasury and staking
5. stakers earn the reward share generated by platform usage

That loop is what makes Arya more than a set of disconnected pages. The contracts and UI are designed to work together as one system.
...
This repository is trying to demonstrate production-readiness, not just isolated contract functions. The important architectural choices are:

- upgradeable contracts so the system can evolve safely on testnet
- inter-contract calls so fee routing is enforced on-chain instead of simulated in the UI
- a shared registry so the frontend and operators have one source of truth for addresses
- a token and treasury model that supports staking, liquidity, and future ecosystem growth
- responsive frontend surfaces so the system is usable on mobile as well as desktop

That combination is why the project includes both smart contracts and product-facing UX instead of stopping at contract tests alone.
